...
static NSString * const kTCDownloadLockName = @"com.ichensheng.networking.download.operation.lock";
...
//    CFRunLoopRun();
    ...
    @weakify(self)
    self.downloadTask =
    [TCFileDownloader downloadFile:self.downloadURL progress:^(NSUInteger total, NSUInteger completed) {
        @strongify(self)
        if (!self) {
            return;
        }
